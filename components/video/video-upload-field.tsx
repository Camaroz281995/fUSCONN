...
import { FileVideo, Upload, X, ImageIcon, Camera, AlertCircle } from "lucide-react"
import {
  Dialog,
  DialogContent,
  DialogHeader,
...
export default function VideoUploadField({ onVideoUploaded }: VideoUploadFieldProps) {
  ...
  // Reduce max file size to 5MB to avoid 413 errors
  ...
  const handleFileChange = useCallback((e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    // Check file size with reduced limit
    if (selectedFile.size > MAX_FILE_SIZE) {
      setError(`File is too large (max ${MAX_FILE_SIZE / (1024 * 1024)}MB). Please select a smaller video.`)
      return
    }
  ...
  const handleUpload = async () => {
    ...
    try {
      ...
      if (file.size > MAX_FILE_SIZE) {
        throw new Error(`File is too large (max ${MAX_FILE_SIZE / (1024 * 1024)}MB). Please select a smaller video.`)
      }
      ...
      xhr.onload = () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          ...
          try {
            ...
            if (xhr.status === 413) {
              throw new Error(
                `File is too large for the server. Please select a smaller video (max ${MAX_FILE_SIZE / (1024 * 1024)}MB).`,
              )
            } else {
      ...
      // Update the API endpoint to use the new route
      xhr.open("POST", "/api/videos/upload-new", true)
      xhr.send(formData)
    ...
    <div className="space-y-4">
      {!file ? (
        <div className="border-2 border-dashed rounded-md p-6 text-center">
          ...
          <p className="text-sm text-muted-foreground mb-4">Upload a video from your device</p>
        ...
        <div className="border rounded-md p-4">
          ...
          <div className="text-xs text-muted-foreground mb-2">
            ...
            {file.size > MAX_FILE_SIZE && (
              <p className="text-red-500 mt-1">File exceeds the maximum size limit. Please select a smaller video.</p>
            )}
          </div>

          {previewUrl && (
            <div className="mb-3 relative aspect-video bg-black rounded-md overflow-hidden">
              <video
                src={previewUrl}
          ...
          {progress === 100 && (
            <Alert className="bg-green-50 border-green-200 text-green-800">
              <AlertDescription className="text-sm">Video uploaded successfully!</AlertDescription>
            </Alert>
          )}
        </div>
      )}
      ...
      <Dialog open={showUploadDialog} onOpenChange={setShowUploadDialog}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              Choose a video from your photo library or record a new one
